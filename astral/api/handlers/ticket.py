        ...
        if ticket.destination == Node.me():
            if self.request.remote_ip == '127.0.0.1':
                ...
                try:
                    ...
                    log.warning("We lost %s and couldn't find a replacement "
                            "to failover -- our stream is dead: %s", ticket,
                            e)
            ...
            log.info("%s is being deleted by the destination, must inform the "
                    "source %s", ticket, ticket.source)
            TicketsAPI(ticket.source.uri()).cancel(ticket.absolute_url())
        if ticket:
            ticket.delete()
            session.commit()
